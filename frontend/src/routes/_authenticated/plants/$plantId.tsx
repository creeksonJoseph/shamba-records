...
import { ArrowLeft, ClipboardList, Layers, MessageSquarePlus, ShieldAlert, ShieldCheck, ShieldOff, Sprout } from "lucide-react";
...
function FlagStatusSection({ plant }: { plant: Plant }) {
  const qc = useQueryClient();
  const [reason, setReason] = useState("");

  const mutation = useMutation({
    mutationFn: (override: "at_risk" | "healthy" | null) =>
      api<Plant>(`/plants/${plant.id}/flag/`, {
        method: "PATCH",
        body: { override, reason: reason || undefined },
      }),
    onSuccess: (_, override) => {
      const msg = override === "at_risk"
      ...
      <CardContent className="py-4">
        ...
        <div className="flex flex-wrap gap-2">
          <Button
            ...
            onClick={() => mutation.mutate("at_risk")}
          >
          ...
          <Button
            ...
            disabled={mutation.isPending}
            onClick={() => mutation.mutate("healthy")}
          >
